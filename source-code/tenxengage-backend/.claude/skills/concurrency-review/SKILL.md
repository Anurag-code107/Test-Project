  ...
  "findings": [
    {
      ...
      "line": 42,
      "rule": "race-condition|visibility|deadlock|async-pitfall|thread-pool|completable-future|virtual-threads",
      "message": "Brief description of the concurrency issue",
      "suggestion": "How to fix it",
      "autoFixable": true
    }
  ],
  "filesReviewed": ["path/to/File.java"],
  "summary": "N concurrency issue(s) found across M file(s)"
}
```

**`status` rules:** `"failed"` if any `critical` or `high` finding present. `"passed"` otherwise.
