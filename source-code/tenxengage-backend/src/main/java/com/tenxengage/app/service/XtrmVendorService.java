...
public class XtrmVendorService {
    ...
    public void dispatch(RedemptionRequest request) {
        // Decision 3: branch by walletType now so company-wallet payout slots in additively.
        // Company payout is DEFERRED in v1 (needs a company beneficiary / SPN — not an individual PAT).
        ...
        TransferFundResult result = xtrmApiClient.transferFund(new TransferFundCommand(
                ...
                sku, giftCardEmail,
                request.getAmount(), currency, "Reward redemption"));
