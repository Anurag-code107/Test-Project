...
public class RedemptionApprovalService {
    ...
    public RedemptionRequestDetailResponse approveRedemption(UUID redemptionId, UUID approverId) {
        ...
        String imageUrl = catalogItemApprove
                .map(i -> i.getImageUrl() != null
                        ? "/api/v1/admin/redemption-catalog/" + i.getId() + "/image" : null)
                .orElse(null);
        String providerImageUrl = catalogItemApprove.map(RedemptionCatalogItem::getProviderImageUrl).orElse(null);

        return RedemptionRequestDetailResponse.from(saved, catalogItemName, imageUrl, providerImageUrl, null, null);
    }
        ...
        if (request.getCategory() == RedemptionCategory.CASH) {
            ...
            try {
                ...
                if (request.getVendorReferenceId() != null) {
                    ...
                    final String beneficiaryTxn = request.getBeneficiaryTransactionId();
                    final var payoutMethod = request.getPayoutMethod();
                    final String payoutLabel = request.getPayoutDestinationLabel();
                    redemptionRequestRepository.findById(request.getId()).ifPresent(fresh -> {
                        fresh.setVendorReferenceId(vendorRef);
                        fresh.setBeneficiaryTransactionId(beneficiaryTxn);
                        fresh.setPayoutMethod(payoutMethod);
                        fresh.setPayoutDestinationLabel(payoutLabel);
                        redemptionRequestRepository.save(fresh);
                    });
                }
            } catch (BusinessRuleException definitive) {
                // Definitive vendor rejection (e.g. send limit) — the transfer did NOT execute. Safe to release
                // the reservation and fail, matching the CASH INSTANT path. Uses the shared settlement (its own
                // transaction, terminal-state guarded) so a late webhook/cron can never double-settle.
                log.warn("[APPROVAL_DISPATCH_REJECTED] redemptionId={}, code={} — releasing reservation",
                ...
                // Ambiguous (timeout / unreachable) — the vendor MAY have accepted the transfer.
    ...
    public RedemptionRequestDetailResponse rejectRedemption(UUID redemptionId,
        ...
        String imageUrlReject = catalogItemReject
                .map(i -> i.getImageUrl() != null
                        ? "/api/v1/admin/redemption-catalog/" + i.getId() + "/image" : null)
                .orElse(null);
        String providerImageUrlReject = catalogItemReject.map(RedemptionCatalogItem::getProviderImageUrl).orElse(null);

        return RedemptionRequestDetailResponse.from(
                saved, catalogItemNameReject, imageUrlReject, providerImageUrlReject, null, null);
    }
