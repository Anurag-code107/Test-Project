...
public class TenantRedemptionCatalogService {
    ...
    public ClientCatalogItemConfigResponse upsertItemConfig(UUID catalogItemId,
        ...
        // Overrides may only NARROW the item's range, never widen it: the floor can move up, the
        // ceiling can move down. Symmetric with the resolution in CatalogBrowseItemResponse.from()
        // and RedemptionSubmissionService.
        ...
        // A null item ceiling means open-value, so any finite override still narrows — allowed.
        if (request.maxTransactionAmountOverride() != null
                && item.getDefaultMaxRedemptionAmount() != null
                && request.maxTransactionAmountOverride()
                        .compareTo(item.getDefaultMaxRedemptionAmount()) > 0) {
            throw new BusinessRuleException(
                    "Maximum transaction amount cannot be set above the catalog item's platform maximum of "
                            + item.getDefaultMaxRedemptionAmount());
        }

        // Both overrides arrive together, so check the resulting range in one place — this also stops a
        // FIXED denomination being squeezed out of its own [min, max] window.
        BigDecimal resultingMin = request.minTransactionAmountOverride() != null
                ? request.minTransactionAmountOverride()
                : item.getDefaultMinRedemptionAmount();
        BigDecimal resultingMax = request.maxTransactionAmountOverride() != null
                ? request.maxTransactionAmountOverride()
                : item.getDefaultMaxRedemptionAmount();
        if (resultingMax != null && resultingMin.compareTo(resultingMax) > 0) {
            throw new BusinessRuleException(
                    "Minimum transaction amount (" + resultingMin
                            + ") cannot exceed the maximum transaction amount (" + resultingMax + ")");
        }
        ...
        if (existing.isPresent()) {
            ...
            config.setMaxTransactionAmountOverride(request.maxTransactionAmountOverride());
            ...
            config = ClientCatalogItemConfig.builder()
                    ...
                    .maxTransactionAmountOverride(request.maxTransactionAmountOverride())
