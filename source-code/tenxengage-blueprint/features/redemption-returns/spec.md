...
> **Post-launch UX update (2026-06-30 · FE UX enhancements CR-01):** in the **My Returns** tab (Transaction History), the per-row "Cancel Return" action renders as a real button (`outline` variant + destructive-tinted border) instead of plain red text, for a clear interactive affordance. Behavior is unchanged (confirm dialog + cancel mutation).
...
No caching on `RedemptionReturn` data. Return status can change at any time via async Xoxoday webhook — cached stale status would mislead both partner and admin views. The 2 min TanStack Query `staleTime` on FE hooks provides the only client-side cache, which is acceptable given the async update pattern.
