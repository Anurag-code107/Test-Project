...
function MyProfilePage() {
  ...
  const canPayout = canAny("action.redemption.redeem", "action.redemption.redeem_company");

  // URL-driven tab selection (?tab=...) so the Payout tab is directly linkable.
  const [searchParams, setSearchParams] = useSearchParams();
  const VALID_TABS = ["profile", "notifications", "privacy", "help", ...(canPayout ? ["payout"] : [])];
  const tabParam = searchParams.get("tab");
  const activeTab = tabParam && VALID_TABS.includes(tabParam) ? tabParam : "profile";
  // Legacy back-compat: the Withdraw tab was merged into Payout (withdraw is now a modal). Redirect old links
  // to Payout — otherwise ?tab=withdraw falls through to Profile.
  useEffect(() => {
    if (tabParam === "withdraw" && canPayout) {
      setSearchParams(
        (prev) => {
  ...
  }, [tabParam, canPayout, setSearchParams]);
    ...
    <div className="flex flex-col h-full gap-6">
      ...
      >
        <TabsList className="self-start shrink-0">
          ...
          {canPayout && <TabsTrigger value="payout">Payout</TabsTrigger>}
        ...
        >
          <div className="relative flex-1 min-h-0 flex flex-col border border-border rounded-lg shadow-sm overflow-hidden">
            ...
            >
              ...
              {/* Mobile number (payout enrollment + withdrawal OTP) */}
              <MobileNumberSection />
        ...
        {canPayout && (
          <TabsContent value="payout" className="mt-0 flex-1 min-h-0 overflow-y-auto">
            <PayoutTab />
          </TabsContent>
        )}
