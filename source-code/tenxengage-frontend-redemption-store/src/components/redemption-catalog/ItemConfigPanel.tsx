...
const catalogItemConfigSchema = z.object({
  ...
  maxTransactionAmountOverride: z.string().optional(),
...
export function ItemConfigPanel({ catalogItemId, enabled, onClose }: Props) {
  ...
  const {
    ...
    defaultValues: {
      ...
      maxTransactionAmountOverride: item?.maxTransactionAmountOverride ?? "",
  ...
  }, [item?.configId, item?.processingModeOverride, item?.minTransactionAmountOverride, item?.maxTransactionAmountOverride, item?.minWalletBalanceOverride, item?.returnWindowDaysOverride, reset]);
  ...
  async function onSubmit(values: FormValues) {
    const request = {
      ...
      maxTransactionAmountOverride: values.maxTransactionAmountOverride || undefined,
      ...
      { catalogItemId, request },
      {
        ...
        onError: (err: unknown) => {
          ...
          if (status === 422) {
            ...
            // The server rejects three range violations (min too low, max too high, min > max).
            // Anchor the message to whichever bound it names so the error lands on that input.
            setError(
              /^maximum/i.test(msg) ? "maxTransactionAmountOverride" : "minTransactionAmountOverride",
              { message: msg },
            );
          } else if (status === 409) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 mt-4" data-testid="item-config-panel">
      ...
      <div className="space-y-1">
        <Label htmlFor="maxTransactionAmountOverride">Max Transaction Amount Override</Label>
        <Input
          id="maxTransactionAmountOverride"
          placeholder={
            item?.defaultMaxRedemptionAmount
              ? `e.g. 500.00 (leave blank to inherit ${item.defaultMaxRedemptionAmount})`
              : "e.g. 500.00 (leave blank for no ceiling)"
          }
          {...register("maxTransactionAmountOverride")}
        />
        {errors.maxTransactionAmountOverride && (
          <p className="text-sm text-destructive" data-testid="max-amount-error">
            {errors.maxTransactionAmountOverride.message}
          </p>
        )}
      </div>
