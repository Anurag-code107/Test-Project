### 10. Approval pause — **gate-conditional**

**Phase routing — checked first, before gate mode:**

If `$PHASE == implement` (set only by run-feature's orchestrator):
1. Run a conditional fresh test: if you arrived here directly from `subagent/step-09.5-ready-check-loop.md` with `status=pass` and `git rev-parse HEAD` matches Step 9.5's final SHA, skip — Step 9.5 ended green on this same commit. Otherwise run `npm run test` AND `npx playwright test` fresh and confirm both pass.
...
- If `$GATE in {story, ready-check, feature-end}`:
  - Run `npm run test` AND `npx playwright test` fresh and verify both pass.
  - If ready-check is green (strict stages): proceed directly to Step 10.5 (ready-check report commit) then Step 11 (squash-merge).
...
1. **Conditional fresh test run.** If you arrived here directly from `subagent/step-09.5-ready-check-loop.md` with `status=pass` (i.e., no `change X` re-entry between Step 9.5 and here, and `git rev-parse HEAD` matches the SHA at the end of Step 9.5), skip the fresh test run — Step 9.5's loop ended green on this same SHA. Otherwise (first entry after `change X`, or any re-entry), run `npm run test` AND `npx playwright test e2e/{feature}.spec.ts` fresh in this message, read full output, confirm both pass with 0 failures. Do not claim "green" without this evidence. If `$USE_TDD = true`, also invoke `superpowers:verification-before-completion`.
...
**Pause-wrap (see `## Time accounting` above).** Immediately before showing the approval message, capture:
...
- On `merge` → read `steps/step-10.5-rc-report-commit.md`.
- On `change X` → implement change, rerun `npm run test` and Playwright, re-read this file.
