...
public class RedemptionWebhookService {
    ...
    private void applyCompletion(RedemptionRequest request,
        ...
        if (resvBefore.compareTo(request.getAmount()) < 0) {
            // Reserved balance insufficient — may have already been released by a failure recovery path.
            // Dead-letter for manual reconciliation rather than writing a negative balance.
            webhookEvent.setStatus(WebhookStatus.DEAD_LETTERED);
            webhookEvent.setFailureReason("Insufficient reserved balance for completion: reserved="
                    + resvBefore + ", amount=" + request.getAmount());
            webhookEventRepository.save(webhookEvent);
            log.error("[step=webhook-insufficient-reserved] redemptionId={}, reserved={}, amount={}",
    ...
    private void applyFailure(RedemptionRequest request,
        ...
        if (resvBefore.compareTo(request.getAmount()) < 0) {
            ...
            webhookEvent.setFailureReason("Insufficient reserved balance for failure settlement: reserved="
                    + resvBefore + ", amount=" + request.getAmount());
            webhookEventRepository.save(webhookEvent);
            log.error("[step=webhook-insufficient-reserved-failure] redemptionId={}, reserved={}, amount={}",
                    request.getId(), resvBefore, request.getAmount());
            return;
        }
