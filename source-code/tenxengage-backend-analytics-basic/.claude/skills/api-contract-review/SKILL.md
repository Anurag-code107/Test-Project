...
1. Apply all relevant checks from the checklist above to the specified controller/DTO files
  ...
  "findings": [
    {
      ...
      "line": 42,
      "rule": "http-verb|versioning|entity-leak|status-code|error-format|pagination|backward-compat|response-consistency",
      "message": "Brief description of the API design issue",
      "suggestion": "How to fix it",
      "autoFixable": true
    }
  ],
  "filesReviewed": ["path/to/Controller.java"],
  "summary": "N API design issue(s) found across M file(s)"
}
```

**`status` rules:** `"failed"` if any `critical` or `high` finding (entity-leak, missing versioning, 200-with-error are always high+). `"passed"` otherwise.
