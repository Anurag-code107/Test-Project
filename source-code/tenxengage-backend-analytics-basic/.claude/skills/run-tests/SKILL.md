...
On exit (success or failure), emit a structured return block matching the v9
convention used by `/load-story` and `/execute-foundation`. Multiple lines,
one field per line, prefixed `ORCHESTRATOR_RETURN`.

Required fields (always emitted):

    ORCHESTRATOR_RETURN status=<success|failure>
    ORCHESTRATOR_RETURN unit_id=<T1-BE if dispatched by /execute-integration-tests else context-derived>
...
Additional fields on failure:
    ...
    ORCHESTRATOR_RETURN failed_class=<test class FQN, or "n/a">
    ORCHESTRATOR_RETURN failed_method=<test method, or "n/a">
    ORCHESTRATOR_RETURN failed_reason=<short reason, single line>
    ORCHESTRATOR_RETURN report_path=<absolute path to gradle test report, or "none">
