...
- Response DTO fields that hold **pre-serialized JSON text** (entity column is `String` storing JSON, produced by `objectMapper.writeValueAsString(...)`) must be declared `String` with `@JsonRawValue` — never `Object`. With `Object`, Jackson re-encodes the string as a JSON string literal, so clients receive `"[{\"id\":...}]"` instead of `[{"id":...}]` and crash on `.map(...)`. Examples: `Question.optionsJson`, `Question.correctAnswerJson`.
