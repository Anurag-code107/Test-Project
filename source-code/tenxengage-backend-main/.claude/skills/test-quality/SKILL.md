...
1. Review the specified test files against the quality checklist above
  ...
  "findings": [
    {
      ...
      "line": 42,
      "rule": "aaa-pattern|naming|assertj|parameterized|soft-assertions|anti-pattern|coverage",
      "message": "Brief description of the test quality issue",
      "suggestion": "How to improve it",
      "autoFixable": false
    }
  ],
  "filesReviewed": ["path/to/FileTest.java"],
  "summary": "N test quality issue(s) found across M test file(s)"
}
```

**`status` rules:** `"failed"` if any `critical` or `high` finding (e.g. swallowed exceptions, no assertions). `"passed"` for medium/low (style, naming).
