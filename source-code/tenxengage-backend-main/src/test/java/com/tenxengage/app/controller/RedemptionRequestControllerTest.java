...
class RedemptionRequestControllerTest {
    ...
    private SubmitPersonalRedemptionRequest validRequest() {
        return new SubmitPersonalRedemptionRequest(CATALOG_ID, WALLET_ID, new BigDecimal("50.00"), "cash");
    }
    ...
    private SubmitCompanyRedemptionRequest validCompanyRequest() {
        return new SubmitCompanyRedemptionRequest(CATALOG_ID, WALLET_ID, new BigDecimal("50.00"), "cash");
    }

    @Test
    @WithMockUser
    void POST_company_201_happyPath() throws Exception {
        withPermission("action.redemption.redeem_company");
        when(submissionService.submitCompanyRedemption(any(), eq(USER_ID)))
                .thenReturn(confirmationResponse());

        mockMvc.perform(post("/api/v1/redemption/requests/company")
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    @Test
    @WithMockUser
    void POST_company_403_missingPermission() throws Exception {
        withoutPermission();
    ...
    @Test
    @WithMockUser
    void POST_company_403_partnerSellerCannotUseCompanyEndpoint() throws Exception {
        withPermission("action.redemption.redeem"); // personal only, not company
    ...
    void GET_200_listRedemptions_paginated() throws Exception {
        ...
        RedemptionRequestResponse item = new RedemptionRequestResponse(
                ...
                "NON_CASH", "INSTANT", Instant.now(), null, null);
        when(submissionService.getPersonalRedemptions(eq(USER_ID), any()))
    ...
    @Test
    @WithMockUser
    void GET_200_getRedemptionById() throws Exception {
        withPermission("action.redemption.view_history");
        RedemptionRequestDetailResponse detail = new RedemptionRequestDetailResponse(
                REQUEST_ID, "RESERVED", new BigDecimal("50.00"), "cash",
                "Test Reward", "NON_CASH", "INSTANT", null, null, Instant.now(), null, null);
        when(submissionService.getRedemptionById(eq(REQUEST_ID), any(UUID.class))).thenReturn(detail);
    ...
    void GET_404_getRedemptionById_wrongTenant() throws Exception {
        withPermission("action.redemption.view_history");
        when(submissionService.getRedemptionById(eq(REQUEST_ID), any(UUID.class)))
