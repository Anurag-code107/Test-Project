...
public class RedemptionSubmissionService {
    ...
    public RedemptionSubmissionConfirmationResponse submitPersonalRedemption(
        ...
        // INSTANT dispatch deferred — blocked on US-05 BE-1 (XTRM not working) and US-06 BE-1
        // (Xoxoday credentials unavailable). INSTANT stays RESERVED until vendor integrations complete.
    ...
    public RedemptionRequestDetailResponse getRedemptionById(UUID id, UUID userId) {
        ...
        String catalogItemName = catalogItemRepository.findById(req.getCatalogItemId())
                ...
                .orElse("(unknown)");
        return RedemptionRequestDetailResponse.from(req, catalogItemName);
    }
