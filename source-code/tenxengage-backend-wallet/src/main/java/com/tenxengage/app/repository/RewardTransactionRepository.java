...
public interface RewardTransactionRepository extends JpaRepository<RewardTransaction, UUID> {
    ...
    /**
     * Sum total awarded for a partner company across an incentive, using a single DB aggregate
     * instead of loading all transactions and filtering in Java.
     */
    @Query("SELECT COALESCE(SUM(rt.amountAwarded), 0) FROM RewardTransaction rt " +
           "JOIN ClaimAction ca ON ca.id = rt.claimActionId " +
           "JOIN PurchaseOrder po ON po.id = ca.purchaseOrderId " +
           "WHERE rt.clientId = :clientId AND rt.incentiveId = :incentiveId " +
           "AND po.partnerCompanyId = :partnerCompanyId")
    ...
    void acquireGrantLock(@Param("lockKey") String lockKey);
