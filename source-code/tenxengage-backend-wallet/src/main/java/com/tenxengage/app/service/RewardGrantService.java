...
public class RewardGrantService {
    ...
    public RewardGrantResult grantReward(RewardGrantRequest request, Incentive incentive) {
        ...
        if (request.partnerCompanyId() != null) {
            ...
            if (partnerCap != null) {
                // TODO: Add sumAwardedByPartnerCompanyAndIncentiveAndCurrency to RewardTransactionRepository
                // For now, use the existing non-currency-scoped query as a fallback.
                // Once the per-currency query is available, replace this call:
                BigDecimal partnerTotal = Optional.ofNullable(rewardTransactionRepository
        ...
        // 5. Credit balance — runs in the same tx as RewardTransaction + BudgetUtilization
