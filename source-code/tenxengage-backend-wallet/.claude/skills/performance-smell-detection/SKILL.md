...
1. Apply all relevant checks from the checklist above to the specified files
  ...
  "findings": [
    {
      ...
      "line": 42,
      "rule": "regex-in-loop|string-concat-in-loop|unbounded-query|parallel-stream-mutable|boxing-in-loop|list-contains-in-loop|stream-in-tight-loop",
      "message": "Brief description of the performance smell",
      "suggestion": "How to fix it",
      "autoFixable": true
    }
  ],
  "filesReviewed": ["path/to/File.java"],
  "summary": "N performance smell(s) found across M file(s)"
}
```

**`status` rules:** `"failed"` if any `critical` or `high` finding present (regex-in-loop, unbounded-query are always high+). `"passed"` otherwise.
