...
class RedemptionWebhookServiceTest {
    ...
    /** Mirrors JPA save: the persisted ledger entry comes back with a generated id. */
    private LedgerEntry savedLedgerEntry(LedgerEntry entry) {
    ...
    void process_completionEvent_writesDebitAndCompletes() {
        ...
        when(ledgerEntryRepository.save(any())).thenAnswer(inv -> savedLedgerEntry(inv.getArgument(0)));
