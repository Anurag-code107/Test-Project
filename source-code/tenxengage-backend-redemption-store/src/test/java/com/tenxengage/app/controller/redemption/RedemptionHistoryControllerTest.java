...
class RedemptionHistoryControllerTest {
    ...
    @Test
    @WithMockUser
    void GET_200_listCompanyRedemptions() throws Exception {
        withPermission("action.redemption.redeem_company");
        when(historyService.getCompanyHistory(eq(USER_ID), any(), any()))
                .thenReturn(new PageImpl<>(List.of(sampleItem())));

        mockMvc.perform(get("/api/v1/redemption/requests/company"))
    ...
    @Test
    @WithMockUser
    void GET_200_listCompanyRedemptions_withStatusFilter() throws Exception {
        withPermission("action.redemption.redeem_company");
        when(historyService.getCompanyHistory(eq(USER_ID), any(), any()))
                .thenReturn(new PageImpl<>(List.of()));

        mockMvc.perform(get("/api/v1/redemption/requests/company?status=COMPLETED"))
                .andExpect(status().isOk());
    }

    @Test
    @WithMockUser
    void GET_422_listCompanyRedemptions_dateFromAfterDateTo() throws Exception {
        withPermission("action.redemption.redeem_company");

        mockMvc.perform(get("/api/v1/redemption/requests/company?dateFrom=2026-06-10&dateTo=2026-06-01"))
                .andExpect(status().isUnprocessableEntity());
    }

    @Test
    @WithMockUser
    void GET_403_listCompanyRedemptions_missingPermission() throws Exception {
        withoutPermission();
    ...
    @Test
    @WithMockUser
    void GET_200_listCompanyRedemptions_emptyWhenNoCompanyWallet() throws Exception {
        withPermission("action.redemption.redeem_company");
        when(historyService.getCompanyHistory(eq(USER_ID), any(), any()))
                .thenReturn(new PageImpl<>(List.of()));
