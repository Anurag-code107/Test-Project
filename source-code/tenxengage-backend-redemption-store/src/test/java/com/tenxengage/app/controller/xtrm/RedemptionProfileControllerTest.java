...
class RedemptionProfileControllerTest {
    ...
    @Test
    @WithMockUser
    void GET_200_companyPermissionAlsoAllowed() throws Exception {
        withPermission("action.redemption.redeem_company");
        when(enrollmentService.getProfileView(USER_ID))
                .thenReturn(PartnerRedemptionFixtures.notEnrolled(CLIENT_ID, USER_ID).build());

        mockMvc.perform(get("/api/v1/redemption/profile"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.enrollmentStatus").value("NOT_ENROLLED"))
