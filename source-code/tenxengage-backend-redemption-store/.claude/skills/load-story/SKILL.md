...
- `--tdd` → set `$USE_TDD = true` (default: `false`).
  - When `true`: the Red/Green/Refactor cycle and the four superpowers skills referenced in Steps 6–8 are active for this session.
...
- `--phase=<implement|merge|revise>` → set `$PHASE` (default: unset, meaning both phases inlined).
  - `implement` — run through Step 8 but STOP before merge; emit structured return summary. Exit cleanly.
  - `merge` — assume Step 8 already produced a green ready sub-branch; jump to Step 8.5.
...
- `--soft-stages=<comma-list>` → passed through to `ready-check` in Step 7.5 (default: empty).

When `$GATE != every`, the skill must NOT prompt for chat approval in Step 8; instead it auto-merges on green ready-check. When `$GATE == every` (default), behavior is unchanged.
...
- Claim the `BE` cell of the story row in `tracker.md`
...
**There is no GitHub PR for this sub-branch.** The only GitHub PR per feature is the final `features/{feature-slug}` → `roadmaps/{slug}` PR (BRD-derived) or `features/{feature-slug}` → `main` PR (standalone), opened manually when the tracker is all-green.
...
Pause-wrap every post-claim interactive prompt (Step 8 approval pause is the main one).
At done flip (Step 11): `ACTIVE_SECS=$(( NOW_EPOCH - CLAIM_TIME_EPOCH - HUMAN_PAUSE_TOTAL_SECS ))`.
...
- `$PHASE == merge` → read `steps/step-08.5-rc-report-commit.md` (assumes sub-branch exists and ready-check has run; then proceed to step-09-squash-merge.md).
