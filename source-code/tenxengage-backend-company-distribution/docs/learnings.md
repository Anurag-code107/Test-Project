...
| Rule | Category | Applied to |
|---|---|---|
| OpenAPI schema fields typed `string format: date-time` that the service can return null for must declare `nullable: true` — omitting it causes generated clients to reject valid null responses; apply to ALL schemas in the file that share the nullable field | adversarial | PROJECT-CONTEXT.md |

## 2026-06-29 — reward-balance-expiration

| Rule | Category | Applied to |
|---|---|---|
| An empty/null `targetUserIds` on a notification event is a tenant-wide role broadcast (NotificationDispatcher notifies every user holding the type's default roles), NOT "no recipients" — narrower-than-role audiences must resolve explicit recipient IDs at publish time or carry+enforce an audience key (e.g. `partnerCompanyId`) on the event and dispatcher | adversarial | docs/patterns/event-publishing.md |
| A scheduled sweep acting on a precomputed work-item must re-validate the *triggering business condition* (not just status + idempotency) from live data under the row lock before an irreversible write, and cancel/reschedule if it no longer holds — precomputed eligibility goes stale between warn and execute | adversarial | docs/patterns/lifecycle-dates.md |
| Compute every date boundary with a fixed zone (`LocalDate.now(ZoneOffset.UTC)`), never the JVM-default `LocalDate.now()`; mixing the two within one feature shifts window edges by a day on non-UTC hosts | lifecycle-dates | docs/patterns/lifecycle-dates.md |
| Offset-paginating a repository query (iterating `PageRequest` page numbers) without a deterministic `ORDER BY` silently skips or double-processes rows across pages — a comment claiming "ordered for stable paging" is not an `ORDER BY`; verify the clause is actually in the JPQL | db-performance | PROJECT-CONTEXT.md |
