 ...
 * and the cron can never double-settle (terminal-state guard). Past the cap it stops polling and alerts for
 * manual review — it never auto-releases a possibly-paid transfer.
 *
 * <p>Covers <b>both</b> individual redemptions and the distribution store's company payout legs — see
 * {@link #RECONCILED_WALLET_TYPES}. This sentence used to read "never touches company payouts (deferred)",
 * which stopped being true when F-8 added COMPANY to that list and was left contradicting the constant
 * eight lines below it.</p>
 */
@Component
public class RedemptionReconciliationService {
    ...
    /**
     * Both wallet types are polled. INDIVIDUAL covers self-service redemptions; COMPANY covers the
     * distribution store's payout legs, which were previously invisible here — a missed XTRM webhook on a
     * distribution left funds reserved on the company wallet indefinitely, and because the same filter
     * guarded the past-cap warning, with no alert either (design F-8).
     */
    private static final List<WalletType> RECONCILED_WALLET_TYPES =
            List.of(WalletType.INDIVIDUAL, WalletType.COMPANY);
        ...
        for (Client client : clients) {
            try {
                List<RedemptionRequest> inFlight = redemptionRequestRepository.findInFlightForReconciliation(
                        client.getId(), RedemptionCategory.CASH, RECONCILED_WALLET_TYPES, IN_FLIGHT, cutoff);
    ...
    private int reconcileSingle(RedemptionRequest r) {
        ...
        // Poll as whoever actually paid. A distribution leg was remitted by its partner company, and the
        // platform cannot see that transaction — res.found() would be false on every run, so the item would
        // never settle and the recipient's share would stay reserved indefinitely.
