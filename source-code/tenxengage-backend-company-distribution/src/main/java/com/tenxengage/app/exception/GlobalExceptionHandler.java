...
public class GlobalExceptionHandler {
    ...
    /**
     * Wrong HTTP method for an existing path — 405, not 500.
     *
     * <p>Without this handler the exception fell through to the catch-all and every wrong-method request
     * answered {@code 500 INTERNAL_SERVER_ERROR}, which reads as a server crash and tells the caller nothing.
     * It became reachable when the company-redemption endpoints were retired: a stale client still POSTing to
     * {@code /redemption/requests/company} now finds no such mapping, and deserves a clean "method not
     * allowed" rather than an apparent outage. {@code Allow} is set from the methods Spring reports.</p>
     */
    ...
    public ResponseEntity<ErrorResponse> handleMethodNotSupported(
        ...
        if (allowed != null && !allowed.isEmpty()) {
            res.allow(allowed.toArray(new HttpMethod[0]));
        }
    ...
    public ResponseEntity<ErrorResponse> handleBusinessRule(
        ...
        log.warn("Business rule violation at {}: errorCode={} message={}", request.getRequestURI(), ex.getErrorCode(), ex.getMessage());
        ErrorResponse error = ErrorResponse.of(
            ex.getErrorCode(),
            ex.getMessage(),
    ...
    public ResponseEntity<ErrorResponse> handleExternalService(
        ...
        log.warn("External service unavailable at {}: errorCode={} message={}",
    ...
    public ResponseEntity<ErrorResponse> handleDataIntegrityViolation(
        ...
        ErrorResponse error = ErrorResponse.of(
            ...
            "The request violates a data constraint, such as a duplicate or missing required value.",
            HttpStatus.CONFLICT.value(),
            request.getRequestURI()
        );
        return ResponseEntity.status(HttpStatus.CONFLICT).body(error);
