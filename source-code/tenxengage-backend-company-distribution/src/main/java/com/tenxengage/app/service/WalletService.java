...
public class WalletService {
    ...
    public List<RewardWalletResponse> getCompanyWallets(UUID companyId) {
        ...
        if (hasRole(user, "PARTNER_ADMIN")) {
            ...
            if (!hasRole(user, "CLIENT_ADMIN") && !hasRole(user, "PLATFORM_ADMIN")) {
                throw new AccessDeniedException("Company wallet access requires CLIENT_ADMIN or PLATFORM_ADMIN role");
            }
    ...
    /**
     * Releases reserved balance back to available within the caller's transaction.
     * Must be called from an active @Transactional context — fails fast with
     * IllegalTransactionStateException if no transaction is present.
     */
    @Transactional(propagation = Propagation.MANDATORY)
    public void releaseReservedBalance(RedemptionRequest request) {
        RewardWallet wallet = rewardWalletRepository.findByIdForUpdate(request.getWalletId())
        ...
        log.info("step=reserved_balance_released walletId={} amount={}", wallet.getId(), amount);
    }
    ...
    /**
     * Reverses a prior credit by reducing available balance within the caller's transaction.
     * Writes a REVERSAL ledger entry for full audit trail. Idempotent on (walletId, referenceType, referenceId).
     */
    @Transactional(propagation = Propagation.MANDATORY)
    public RewardWallet reversalInCurrentTx(UUID clientId, UUID userId, String currencyId,
                                            BigDecimal amount, String referenceType, UUID referenceId) {
        RewardWallet wallet = rewardWalletRepository
            .findForUpdate(clientId, userId, currencyId, WalletType.INDIVIDUAL)
            .orElseThrow(() -> new ResourceNotFoundException("RewardWallet", "userId", userId));

        if (referenceId != null && ledgerEntryRepository
                .existsByRewardWalletIdAndReferenceTypeAndReferenceIdAndEntryType(
                    wallet.getId(), referenceType, referenceId, LedgerEntryType.REVERSAL)) {
            log.info("step=duplicate_reversal_skipped walletId={} referenceType={} referenceId={}",
                wallet.getId(), referenceType, referenceId);
            return wallet;
        }
        ...
        if (wallet.getAvailableBalance().compareTo(amount) < 0) {
            throw new BusinessRuleException(
                "Insufficient available balance for reversal on currency " + currencyId);
        }
        ...
        wallet.setAvailableBalance(availAfter);
        RewardWallet saved = rewardWalletRepository.save(wallet);
        log.info("step=balance_reversed walletId={} amount={} currencyId={}",
            saved.getId(), amount, currencyId);
        return saved;
    }
