...
public class RedemptionHistoryService {
    ...
    public RedemptionRequestDetailResponse getRedemptionDetail(UUID id, UUID userId) {
        ...
        RedemptionRequest req = isAdmin
                ...
                : redemptionRequestRepository.findByIdAndClientIdAndUserIdAndOrigin(
                        id, clientId, userId, RedemptionOrigin.SELF)
                        .orElseThrow(() -> new ResourceNotFoundException("RedemptionRequest", "id", id));

        var detailCatalogItem = catalogItemRepository.findById(req.getCatalogItemId());
        String catalogItemName = detailCatalogItem.map(RedemptionCatalogItem::getName).orElse("(unknown)");
        // The card renders the uploaded image if there is one, else the SKU's brand image, else the
        // category illustration — the same precedence as the store grid.
        String imageUrl = detailCatalogItem
                .map(item -> item.getImageUrl() != null
                        ? "/api/v1/admin/redemption-catalog/" + item.getId() + "/image" : null)
                .orElse(null);
        String providerImageUrl = detailCatalogItem.map(RedemptionCatalogItem::getProviderImageUrl).orElse(null);

        // Resolve the reviewer's display name (approver of a COMPLETED/CANCELLED request) from its id.
        String reviewedByName = req.getReviewedBy() == null ? null
                : userRepository.findById(req.getReviewedBy())
                        .map(u -> (u.getFirstName() + " " + u.getLastName()).trim())
                        .filter(n -> !n.isEmpty())
                        .orElse(null);

        // linkedReturnId: null until F-06 deploy
        return RedemptionRequestDetailResponse.from(
                req, catalogItemName, imageUrl, providerImageUrl, reviewedByName, null);
    }
    ...
    public Page<RedemptionAdminHistoryResponse> getTenantHistory(
        ...
        Page<RedemptionRequest> page = historyRepository.findTenantHistory(
                ...
                filters.userId(),
                filters.companyId(),
                blankToNull(filters.userName()),
                blankToNull(filters.companyName()),
                base.status(),
                base.category(),
    ...
    /** Treat blank filter strings as "no filter" so an empty search box matches everything. */
