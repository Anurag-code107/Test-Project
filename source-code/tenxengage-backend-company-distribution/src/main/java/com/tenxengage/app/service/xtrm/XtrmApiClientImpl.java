...
public class XtrmApiClientImpl implements XtrmApiClient {
    ...
    /**
     * Access tokens, keyed by OAuth client id.
     *
     * <p>A single token pair was correct while every call authenticated as the platform. It stopped being
     * correct the moment partner companies got their own credentials: one company's token would be handed to
     * another company's call, and XTRM would either reject it or — worse — accept it and move the wrong
     * account's money. Keyed by client id, a token can only ever be replayed for the account it was issued
     * to.</p>
     */
    ...
    private record CachedToken(String token, Instant expiry) {
        boolean usableNow() {
            // A minute of headroom so a token cannot expire mid-flight between the check and the call.
            return token != null && Instant.now().isBefore(expiry.minusSeconds(60));
        }
    }

    // ---------------------------------------------------------------------
    // CreateBeneficiary (a beneficiary COMPANY)
    // ---------------------------------------------------------------------
    ...
    public CreateBeneficiaryResult createBeneficiary(CreateBeneficiaryCommand cmd) {
        ...
        // Capital-R "Request" here, matching the sandbox-verified call — not the lowercase envelope() helper.
    ...
    /**
     * Split out from the HTTP call so the part that can lose a one-shot secret is testable without a network.
     */
    CreateBeneficiaryResult parseCreateBeneficiary(Map<?, ?> response) {
        ...
        if (isBlank(spn) || isBlank(clientId) || isBlank(secret)) {
            // NOT retryable. XTRM reported success, so the account exists and its name is taken — a retry
            // fails on the duplicate and the secret is gone either way. Surface it for support rather than
            // looping against a call that can never succeed again.
    ...
    public CreateUserResult createUser(CreateUserCommand cmd, XtrmCredentials credentials) {
        ...
        Map<String, Object> request = new LinkedHashMap<>();
        // The binding field. It must agree with the token below, or XTRM binds the user to the wrong
        // account — permanently, since the email cannot be reused under another issuer.
    ...
    public TransferFundResult transferFund(TransferFundCommand cmd) {
        return transferFund(cmd, platformCredentials());
    }
    ...
    public TransferFundResult transferFund(TransferFundCommand cmd, XtrmCredentials credentials) {
        ...
        Map<String, Object> transaction =
                baseTransaction(credentials, cmd.paymentMethodId(), cmd.currency(), cmd.description());
        ...
        Map<?, ?> response;
        try {
            response = post("/API/v4/Fund/TransferFund", body, credentials);
        } catch (RuntimeException e) {
    ...
    public TransactionStatusResult getTransactionDetails(GetTransactionDetailsCommand cmd,
        ...
        Map<String, Object> request = new LinkedHashMap<>();
        // Both of these must be the remitter's, not the platform's: the account named in the body and the
        // account the bearer token belongs to have to agree, or XTRM reports the transaction as not found —
        // which reconciliation cannot distinguish from "still in flight".
        request.put("IssuerAccountNumber", credentials.issuerAccountNumber());
    ...
    public BatchStatusResult getBatchStatus(GetBatchStatusCommand cmd) {
        return getBatchStatus(cmd, platformCredentials());
    }
    ...
    protected Map<?, ?> post(String path, Map<String, Object> body) {
        return post(path, body, platformCredentials());
    }
    ...
    protected Map<?, ?> post(String path, Map<String, Object> body, XtrmCredentials credentials) {
        ...
        ResponseEntity<Map> resp = restClient.post()
                .uri(baseUrl + path)
                .header("Authorization", "Bearer " + getAccessToken(credentials))
    ...
    protected Map<?, ?> get(String path) {
        return get(path, platformCredentials());
    }
    ...
    /** The platform's own credentials — what every call used before partner companies could pay. */
    XtrmCredentials platformCredentials() {
        return new XtrmCredentials(clientId, clientSecret, issuerAccountNumber, walletId, programId);
    }

    synchronized String getAccessToken() {
        return getAccessToken(platformCredentials());
    }

    synchronized String getAccessToken(XtrmCredentials credentials) {
        CachedToken cached = tokenCache.get(credentials.clientId());
        if (cached != null && cached.usableNow()) {
            return cached.token();
        }
        // Log the account, never the secret — this line fires on every payout for a newly-active company.
        log.info("[step=xtrm_token_fetch] fetching new OAuth2 token for clientId={}", credentials.clientId());
        ...
        tokenCache.put(credentials.clientId(), new CachedToken(token, Instant.now().plusSeconds(expiresIn)));
        return token;
    }
    ...
    /**
     * The remitter half of a payment: who is paying, and from which wallet.
     *
     * <p>Both come from {@code credentials}, not from configuration, because XTRM validates the wallet
     * against the authenticated account. Reading either from a field here would let a company-authenticated
     * call quietly spend the platform's wallet.</p>
     */
    private Map<String, Object> baseTransaction(XtrmCredentials credentials, String paymentMethodId,
                                                String currency, String description) {
        Map<String, Object> transaction = new LinkedHashMap<>();
        transaction.put("IssuerAccountNumber", credentials.issuerAccountNumber());
        transaction.put("PaymentType", "Personal");
        transaction.put("PaymentMethodID", paymentMethodId);
        transaction.put("WalletID", credentials.walletId());
        ...
        transaction.put("ProgramID", credentials.programId());
        return transaction;
    }
