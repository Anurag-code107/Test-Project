...
```json
{
  ...
  "findings": [
    {
      ...
      "line": 42,
      "rule": "null-safety|exception-handling|collections|concurrency|idioms|resources|api-design|performance",
  ...
  "summary": "N issue(s) found across M file(s)"
}
```

**`status` rules:** `"failed"` if any `critical` or `high` finding present. `"passed"` otherwise (medium/low findings still reported as warnings).
