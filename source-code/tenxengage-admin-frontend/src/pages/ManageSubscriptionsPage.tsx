...
import { Plus, Loader2, Shield, Sparkles, ChevronDown } from "lucide-react";
...
const CATEGORY_ORDER = ["general", "branding", "integrations", "analytics", "ai", "incentives", "compliance"];
...
export default function ManageSubscriptionsPage() {
  ...
  const groupedFlags = useMemo(() => {
    ...
    const groups = new Map<string, FeatureFlagResponse[]>();
    ...
    return CATEGORY_ORDER
      ...
      .map((cat) => ({ category: cat, label: getCategoryLabel(cat), flags: groups.get(cat)! }))
      .concat(
        [...groups.entries()]
          .filter(([cat]) => !CATEGORY_ORDER.includes(cat))
          .map(([cat, flags]) => ({ category: cat, label: getCategoryLabel(cat), flags }))
      );
  }, [featureFlags]);
  ...
  async function handleCreate() {
    ...
    if (!FEATURE_KEY_PATTERN.test(trimmedKey)) {
      toast.error("Feature key must start with a lowercase letter and contain only lowercase letters, numbers, and underscores");
      return;
    }
    ...
    return (
      <div className="flex flex-col h-full pb-6">
        ...
        <div className="flex items-center justify-center py-12">
          ...
          <span className="ml-2 text-sm text-muted-foreground">Loading feature flags...</span>
        </div>
      </div>
    );
  }
  ...
  const isCreateFormValid = newFeatureKey.trim().length > 0 && FEATURE_KEY_PATTERN.test(newFeatureKey.trim());
      ...
      {groupedFlags.length > 0 ? (
                    ...
                    {group.flags.map((flag) => (
                      <TableRow key={flag.id}>
                        <TableCell className="font-medium">
                          <div className="flex items-center gap-2">
                            <span className="text-sm">{formatFeatureKey(flag.featureKey)}</span>
                              ...
                              <div className="flex items-center justify-center gap-1.5">
                                {isUpdating ? (
                                  ...
                                  <Switch
                                    ...
                                    onCheckedChange={() => handleToggle(flag, tier.key)}
        ...
        <Card>
          <CardContent className="py-12 text-center text-muted-foreground">
            No feature flags configured. Click "Add Feature" to create one.
          </CardContent>
        </Card>
      )}
      ...
      {totalFeatures > 0 && (
        <div className="grid grid-cols-3 gap-4 mt-6">
          {TIERS.map((tier) => {
            ...
            const percentage = totalFeatures > 0 ? (enabledCount / totalFeatures) * 100 : 0;
