...
export function useSetClientOverrides() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ clientId, overrides }: { clientId: string; overrides: SetFeatureOverrideRequest[] }) =>
      clientService.setClientOverrides(clientId, overrides),
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({ queryKey: ["client-overrides", variables.clientId] });
    },
  });
}
