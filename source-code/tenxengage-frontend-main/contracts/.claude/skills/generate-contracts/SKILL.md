...
2. **Read the spec**: `../tenxengage-blueprint/features/{feature-id}/spec.md`
   - If not found: list available features from `../tenxengage-blueprint/registry.md` and ask the user to confirm
...
- **Determine the base branch:**
  - Read `../tenxengage-blueprint/registry.md` — find the row for `{feature-id}` and extract the `Roadmap` column value as `$ROADMAP_SLUG`.
  - If `$ROADMAP_SLUG` is non-empty (feature is BRD-derived):
  ...
  - Otherwise (standalone feature — `Roadmap` column is `—` or empty): `BASE_BRANCH="${FEATURE_BASE_BRANCH:-main}"`
