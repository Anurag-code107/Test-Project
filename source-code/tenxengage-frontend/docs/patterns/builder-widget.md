...
> **Incentive builder pattern.**
>
> This file describes the frontend builder component architecture for the
> **incentive builder** (SALES, TRAINING, ACTIVITY, JOURNEY incentive types).
> See [incentive.md](../../../tenxengage-blueprint/docs/patterns/domains/incentive.md)
> in the domain registry for the full slot-filler table.
>
> **Implementing a feature for a new enablement domain (course, learning path, future)?**
> Use [`builder-widget-platform.md`](builder-widget-platform.md) instead — that file
> documents the enablement builder's shell + module architecture, which is the
> pattern for all enablement types.
> Reference implementation: `src/components/course-builder/CourseBuilderLayout.tsx`.
...
**Background refetch overwrites in-progress edits.** TanStack Query can refetch `courseData` (or any entity data) on window re-focus, invalidation, or stale-time expiry while the user is actively editing. If the Layout component dispatches `HYDRATE`/`LOAD_*` unconditionally on every `useQuery` data change, the form fields will silently revert, and `isDirty` will be reset to `false`. Always guard the hydration `useEffect` with `!state.isDirty` so only the first successful fetch (or a fresh load on `routeId` change) triggers hydration:

```ts
useEffect(() => {
  if (entityData && mode === "edit" && !state.isDirty) {
    dispatch({ type: "HYDRATE", payload: entityData });
  }
}, [entityData, mode, dispatch, state.isDirty]);
```

**Edit-mode save must use the URL param, not the reducer's entity ID.** If the initial GET fails (404, 403, network error), the reducer's `state.entityId` (or `courseId`) is never populated. A save guard written as `if (mode === "edit" && state.entityId)` will silently fall through to the create branch and produce a duplicate entity. Always use the URL param (`routeId`) for the edit-mode guard:

```ts
if (mode === "edit" && routeId) {
  // PUT existing entity — will 4xx if the initial GET truly failed
} else {
  // POST new entity — only reached when mode === "new"
}
```
