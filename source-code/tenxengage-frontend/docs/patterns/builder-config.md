# Builder Configuration Pattern — Frontend (Incentive Builder)

This file documents how the frontend consumes the incentive builder's
`BuilderSectionConfig` / `BuilderFieldConfig` API. For the enablement builder
(courses, learning paths, and future enablement types), see the
[blueprint canonical](../../../tenxengage-blueprint/docs/patterns/builder-config.md)
and the consumer-side pattern (`useBuilderDefinition`, `ACTION_SECTIONS`, SetupHeader)
in [`builder-widget-platform.md`](builder-widget-platform.md).
