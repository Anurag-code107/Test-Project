...
describe("RoleSidebar — consolidated Redemption nav (CR-03/CR-04)", () => {
  ...
  it("Partner Seller: Redemption group with Store + Redemption History; non-redemption nav intact", () => {
    ...
    expect(screen.getByText("Redemption History")).toBeDefined();
  ...
  it("Partner Admin: module + redeem unlocks the storefront", () => {
    mockCan.mockImplementation(
      canWith(
        "module.home",
  ...
  it("Client Admin: no Redemption Store (cannot redeem) but sees the admin sub-items", () => {
    ...
    expect(screen.queryByText("Redemption History")).toBeNull(); // no view_history (personal)
