...
export async function getClients(
  ...
  const response = await api.get<
    ApiResponse<PaginatedResponse<ClientResponse>>
  >("/clients", { params });
...
export async function createClient(
  ...
  const response = await api.post<ApiResponse<ClientResponse>>(
    "/clients",
    data,
  );
  return response.data.data;
...
export async function getClientStats(): Promise<ClientStatsResponse> {
  const response =
    await api.get<ApiResponse<ClientStatsResponse>>("/clients/stats");
  return response.data.data;
}
...
export async function getClientOverrides(
  ...
  const response = await api.get<ApiResponse<ClientFeatureOverrideResponse[]>>(
