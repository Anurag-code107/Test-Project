...
import DistributionHistoryPage from "@/pages/distribution/DistributionHistoryPage";
import CompanyAwardsPage from "@/pages/distribution/CompanyAwardsPage";
  ...
  return (
    <Routes>
      ...
      {/* Storefront + redeem-confirmation require BOTH module.redemption_store (so disabling the
          module hides the store — matches the BE gate on the store endpoints) AND
          action.redemption.redeem, so a Client Admin who holds the module umbrella but cannot
          redeem still can't reach the storefront (CR-04). A Partner Admin passes on that same key
          (granted in V52) — the company variant was retired with company-wallet redemption. */}
      <Route
        element={
          <ProtectedRoute
      ...
      {/* Company distribution. Each route carries the single permission that also gates its sidebar
          item, so a seller can never reach the store and an admin can never reach Company Awards —
          the role split falls out of the permissions rather than needing role checks. */}
      <Route element={<ProtectedRoute permission="action.redemption.distribute" />}>
        <Route element={<AppLayout />}>
          <Route path="/redemption/distribution" element={<DistributionStorePage />} />
          {/* Same gate as the store: whoever can send rewards is exactly who has to set up the payout
              account first. */}
          <Route
            path="/redemption/distribution/payout-setup"
            element={<CompanyPayoutSetupPage />}
          />
        </Route>
      </Route>

      <Route element={<ProtectedRoute permission="action.redemption.view_distribution_history" />}>
        <Route element={<AppLayout />}>
          <Route path="/redemption/distribution/history" element={<DistributionHistoryPage />} />
        </Route>
      </Route>

      <Route element={<ProtectedRoute permission="action.redemption.view_company_awards" />}>
        <Route element={<AppLayout />}>
          <Route path="/redemption/awards" element={<CompanyAwardsPage />} />
        </Route>
      </Route>
