...
1. Load `AGENTS.md` (or relevant rule files from `rules/`) and apply checks to the specified files
  ...
  "findings": [
    {
      ...
      "line": 42,
      "rule": "async-parallel|bundle-barrel-imports|rerender-memo|rerender-dependencies|rerender-derived-state|js-index-maps|rendering-conditional-render",
      "message": "Brief description of the React pattern issue",
      "suggestion": "How to fix it with example",
      "autoFixable": true
    }
  ],
  "filesReviewed": ["src/components/Feature.tsx"],
  "summary": "N React pattern issue(s) found across M file(s)"
}
```

**`status` rules:** `"failed"` if any `critical` or `high` finding (waterfalls, barrel imports in hot paths). `"passed"` otherwise.
